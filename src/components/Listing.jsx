  ...
  return (
    <div id="recipe__list">
          ...
          {Object.keys(props.recipes).length !== 0 ? (
            Object.keys(props.recipes).map((recipe) => (
              <li key={props.recipes[recipe].id}>
                <span
                  onClick={(e) => props.readRecipe(props.recipes[recipe].id)}
                  title={props.recipes[recipe].name}
                >
                  {props.recipes[recipe].name.length > 30
                    ? `${props.recipes[recipe].name.slice(0, 30)}...`
                  ...
                  <Icon
                    ...
                    onClick={() => {
                      const decision = window.confirm(
                        `Are you sure you want to delete the ${props.recipes[recipe].name}'s recipe?`
                      );

                      if (decision) {
                        props.deleteRecipe(props.recipes[recipe].id);
                      }
      ...
      >
        ...
        <Row gutter={16}>
          <Col span={12}>
            <label htmlFor="recipe__ingredients">Recipe Ingredients 1</label>
            <TextArea
              rows={10}
          ...
          <Col span={12}>
            <label htmlFor="recipe__description">Recipe Ingredient 2</label>
            <TextArea
              rows={10}
